                    ...
                    match msg {
                        Ok(txt) => {
                            ...
                            if newdata_len == 0 {
                                ...
                                println!("data init byte len:{}", cloned_txt);
                            ...
                            if (content.len() as u32) >= newdata_len {
                                ...
                                println!("final content len:{}",content.len()  );
                                let param = content.split_off( (newdata_len as usize));
                                

                                println!("databt len {}, param {}",content.len(),param);

                            let info = format!("GETWASM {} {}",param,content);


                                p1clone.send(info.clone()).await.unwrap();
