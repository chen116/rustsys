...
pub async fn run(listener: TcpListener, addr: String ) -> Result<(), Box<dyn Error>>  {
...
async fn process(
    ...
    lines.send(addr.to_string()+" Please enter your username:").await?;

    // Read the first line from the `LineCodec` stream to get the username.
    let username = match lines.next().await {
        Some(Ok(line)) => 
        {
        println!("getting line {}",line);
        line},
